...
pub trait ParseMode: Sized + Debug {
    type Ret<'input>: Clone + Debug + Default + PartialEq;
    type Err;
...
pub enum Piece<'input, R> {
    /// A literal string which should directly be emitted
    Lit(&'input str),
    /// This describes that formatting should process the next argument (as
    /// specified inside) for emission.
    NextArgument(Box<Argument<'input, R>>),
}

/// Representation of an argument specification.
#[derive(Clone, Debug, PartialEq)]
pub struct Argument<'input, R> {
...
impl<'input, R: Default + PartialEq> Argument<'input, R> {
    pub fn is_identifier(&self) -> bool {
        matches!(self.position, Position::ArgumentNamed(_)) && self.format == R::default()
    }
}
...
pub struct Parser<'input, M: ParseMode> {
    mode: PhantomData<M>,
    /// Input to be parsed
    input: &'input str,
...
impl<'input, M: ParseMode> Iterator for Parser<'input, M> {
    type Item = Piece<'input, M::Ret<'input>>;
...
impl<'input, M: ParseMode> Parser<'input, M> {
    ...
    fn argument(&mut self) -> Argument<'input, M::Ret<'input>> {
        ...
        let format = M::parse_modifier(self);
...
const _: () = {
    rustc_index::static_assert_size!(Piece<'_, InlineAsm>, 16);
    rustc_index::static_assert_size!(Piece<'_, Diagnostic>, 16);
    rustc_index::static_assert_size!(Piece<'_, Format>, 16);
};
